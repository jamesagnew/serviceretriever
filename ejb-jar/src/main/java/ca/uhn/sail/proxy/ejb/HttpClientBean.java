...
public class HttpClientBean implements IHttpClient {
	...
	public HttpResponseBean post(IResponseValidator theResponseValidator, UrlPoolBean theUrlPool, String theContentBody, Map<String, String> theHeaders, String theContentType) {
		...
		params.setIntParameter(CoreConnectionPNames.CONNECTION_TIMEOUT, (int) theUrlPool.getConnectTimeoutMillis());
		params.setIntParameter(CoreConnectionPNames.SO_TIMEOUT, (int) theUrlPool.getReadTimeoutMillis());
		...
		doPost(retVal, theResponseValidator, theHeaders, postEntity, client, url);

		if (retVal.getSuccessfulUrl() == null) {
			for (String nextUrl : theUrlPool.getAlternateUrls()) {
				doPost(retVal, theResponseValidator, theHeaders, postEntity, client, nextUrl);
				if (retVal.getSuccessfulUrl() != null) {
					break;
				}
			}
	...
	private void doPost(HttpResponseBean theResponse, IResponseValidator theResponseValidator, Map<String, String> theHeaders, HttpEntity postEntity, DefaultHttpClient client, String url) {
		...
		try {
			...
			if (validates.isValidates() == false) {
				...
				ourLog.info("Failed to invoke service at URL[{}]: {}", url, validates.getFailureExplanation());
