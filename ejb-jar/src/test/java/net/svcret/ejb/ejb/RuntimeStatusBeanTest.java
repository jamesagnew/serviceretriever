...
public class RuntimeStatusBeanTest {
	...
	public void testRecordInvocationMethodAnon() throws ParseException {
		...
		PersServiceVersionStatus persServiceVersionStatus = new PersServiceVersionStatus();
		persServiceVersionStatus.setPid(4L);
		when(svcVer.getStatus()).thenReturn(persServiceVersionStatus);
		...
		ArgumentCaptor<List> invocStatsCapt = ArgumentCaptor.forClass(List.class);
		verify(pers, times(1)).saveInvocationStats(invocStatsCapt.capture());

		ArgumentCaptor<ArrayList> versionStatsCapt = ArgumentCaptor.forClass(ArrayList.class);
		verify(pers, times(1)).saveServiceVersionStatuses(versionStatsCapt.capture());

		verifyNoMoreInteractions(pers);

		List<PersServiceVersionStatus> versionStatuses = versionStatsCapt.getValue();
		assertEquals(1, versionStatuses.size());
		assertEquals(ts2, versionStatuses.get(0).getLastSuccessfulInvocation());

		List<BasePersInvocationStats> value = invocStatsCapt.getValue();
